...
std::vector<Package> Executer::normalizedRequirements(const Package& package) const {
    ...
    if (const std::optional<PluginBundleLayout> layout = plugin_bundle_find_installed(this->config, package.system);
        layout.has_value()) {
...
void Executer::reconcileInstalledOwnership(const std::vector<TaskGroup>& allTaskGroups,
    ...
    auto sameRecordPackage = [](const Package& package, const TransactionRecord& record) {
        return package.action == record.action && package.system == record.system &&
               package.name == record.packageName && package.version == record.packageVersion;
    };
    auto findPackageByRecord = [&](const std::vector<Package>& packages,
                                   const TransactionRecord& record) -> const Package* {
    ...
    auto mergeOwnershipForPackage = [&](const Package& package) {
        ...
        for (const Package& candidate : allPackages) {
            if (candidate.system == package.system && candidate.name == package.name &&
                candidate.version == package.version && candidate.action == package.action) {
                continue;
            }
    ...
    auto wasPlanned = [&](const Package& package) {
        return std::any_of(plannedPackages.begin(), plannedPackages.end(),
                           [&](const Package& candidate) { return samePackage(candidate, package); });
    };
...
Executer::removeOrphanedDependencies(const std::vector<InstalledEntry>& installedState,
                                     const std::vector<TransactionRecord>& records) const {
    if (this->historyManager == nullptr || !this->config.history.trackInstalled) {
        return {};
    }
...
bool Executer::syncInstalledStateForSystem(const std::string& system, const bool allowEmpty) const {
    if (this->historyManager == nullptr || !this->config.history.trackInstalled || system.empty() ||
        this->securityGateway.isGatewaySystem(system)) {
        return false;
    }
...
void Executer::recordHistory(const std::vector<TransactionRecord>& records) const {
    ...
    for (const HistoryEntry& entry : entries) {
        const bool isMutatingAction = entry.action == "install" || entry.action == "ensure" ||
