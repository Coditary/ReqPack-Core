...
inline DiagnosticMessage resolution_failure_diagnostic(const std::string& details, const std::string& scope) {
    return make_error_diagnostic("executor", "Request resolution failed",
                                 "ReqPack could not map requested input to an executable plugin request.",
                                 "Check system name, package specifier, and flags, then retry.", details, "executor",
                                 scope);
}

inline DiagnosticMessage system_update_failure_diagnostic(const std::string& system) {
    return make_error_diagnostic("executor", "System-wide package update failed",
...
inline DiagnosticMessage transaction_update_failure_diagnostic(const std::string& system) {
    return make_error_diagnostic("executor", "Transaction database update failed",
                                 "ReqPack could not persist running state for this package operation.",
                                 "Check transaction database permissions and configured transaction path.", {}, system,
                                 "transaction");
}

inline DiagnosticMessage plugin_group_failure_diagnostic(const std::string& system, const std::string& summary,
                                                         const std::string& cause) {
    return make_error_diagnostic(
        "plugin", summary, cause,
        "Check plugin installation, registry metadata, and package manager health, then retry command.", {}, system,
        "plugin");
}

inline DiagnosticMessage package_failure_diagnostic(const std::string& system, const std::string& packageName,
                                                    const bool unavailable) {
    if (unavailable) {
        return make_error_diagnostic(
            "plugin", "Package is unavailable: " + system + ":" + packageName,
            "Plugin reported that requested package could not be found or provided by current repositories.",
            "Verify package name, repositories, and system support, then retry.", {}, system + ":" + packageName,
            "package");
    }
    return make_error_diagnostic(
        "plugin", "Plugin action failed for " + system + ":" + packageName,
        ...
        system + ":" + packageName, "package");
}

inline DiagnosticMessage security_gateway_finding_diagnostic(const ValidationFinding& finding) {
    const std::string summary =
        finding.message.empty() ? (finding.id.empty() ? finding.kind : finding.id) : finding.message;

    if (finding.kind == "sync_warning") {
        return make_warning_diagnostic(
            "security", summary, "Security gateway could not use preferred refresh path for requested ecosystems.",
            "Check upstream vulnerability feed availability. ReqPack may fall back to a slower full refresh.", {},
            finding.source, "gateway");
    }

    if (finding.kind == "sync_error") {
        return make_error_diagnostic(
            "security", summary, "Security gateway could not prepare vulnerability data required for this command.",
            "Check gateway backend configuration, OSV feed access, and local security index permissions.", {},
            finding.source, "gateway");
    }

    return make_error_diagnostic("security", summary, "Security gateway returned an unexpected validation finding.",
                                 "Inspect security gateway configuration and retry command.", {}, finding.source,
                                 "gateway");
}
...
inline bool samePackage(const Package& left, const Package& right) {
    return left.action == right.action && left.system == right.system && left.name == right.name &&
           left.version == right.version && left.sourcePath == right.sourcePath &&
...
inline bool actionUsesDesiredStateFilter(const ActionType action) {
    return action == ActionType::INSTALL || action == ActionType::ENSURE || action == ActionType::REMOVE ||
...
inline DisplayMode displayModeFromAction(const ActionType action) {
    ...
    case ActionType::INSTALL:
        return DisplayMode::INSTALL;
    case ActionType::ENSURE:
        return DisplayMode::ENSURE;
    case ActionType::REMOVE:
        return DisplayMode::REMOVE;
