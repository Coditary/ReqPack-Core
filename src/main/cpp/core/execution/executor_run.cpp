...
bool Executer::execute(Graph* graph) {
    ...
    if (this->config.execution.useTransactionDb) {
        ...
        if (!recoveryTaskGroups.empty() && !recoveryRunId.empty()) {
            const std::vector<TransactionRecord> recoveryRecords =
                this->executeRecordedTaskGroups(recoveryTaskGroups, recoveryRunId);
    ...
    if (jsonOutput) {
        ...
        emit_run_result_json(document);
               ...
               (!jsonOutput && requestedItemCount > 0 && taskGroups.empty() && !allTaskGroups.empty())) {
        if (!sessionBegun) {
            std::vector<std::string> itemIds;
