...
std::vector<Executer::TransactionRecord> Executer::executeRecordedTaskGroups(const std::vector<TaskGroup>& taskGroups,
    ...
    auto has_failure = [](const std::vector<TransactionRecord>& groupRecords) {
        return std::any_of(groupRecords.begin(), groupRecords.end(),
                           [](const TransactionRecord& record) { return record.status == "failed"; });
    };
        ...
        for (;;) {
            ...
            TaskGroup taskGroup;
            {
                ...
                for (;;) {
                    ...
                    for (std::size_t attempt = 0; attempt < readyCount; ++attempt) {
                        ...
                        if (state.started[candidate] || state.failed[candidate] ||
                            state.pendingDependencies[candidate] != 0) {
                            continue;
                        }
...

            {
                ...
                for (const std::size_t successor : plans[index].successors) {
                    ...
                    if (state.pendingDependencies[successor] == 0 && !state.started[successor] &&
                        !state.completed[successor] && !state.failed[successor]) {
