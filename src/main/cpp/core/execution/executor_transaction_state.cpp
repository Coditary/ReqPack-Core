...
void Executer::writeTransactionResults(const std::vector<TransactionRecord>& records) const {
    ...
    for (const TransactionRecord& record : records) {
        ...
        for (const TransactionItemRecord& item : runItems) {
            if (item.package.action == package.action && item.package.system == package.system &&
                item.package.name == package.name && item.package.version == package.version) {
        ...
        (void)this->transactionDatabase->updateItemStatus(this->activeRunId, package, record.status,
                                                          record.errorMessage);
    }
}
