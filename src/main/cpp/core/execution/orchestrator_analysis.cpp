...
bool has_explicit_version(const std::string& packageSpecifier) {
    ...
    return versionSeparator != std::string::npos && versionSeparator != 0 &&
           versionSeparator + 1 < packageSpecifier.size();
}
...
DiagnosticMessage validation_blocked_finding_diagnostic(const ValidationFinding& finding) {
    ...
    if (finding.kind == "sync_error") {
        return make_error_diagnostic(
            "security", summary, "ReqPack could not refresh vulnerability data required by current security policy.",
            "Check OSV feed access, local vulnerability database permissions, and retry.", {}, finding.source, "sync");
    }

    if (finding.kind == "unsupported_ecosystem") {
        return make_error_diagnostic("security", summary,
                                     "ReqPack has no vulnerability ecosystem mapping for requested package system.",
                                     "Configure `security.ecosystemMap` or plugin `osvEcosystem`, or relax strict "
                                     "ecosystem mapping if risk is acceptable.",
                                     {}, finding.source, "mapping");
    }

    if (finding.kind == "unresolved_version") {
        return make_error_diagnostic(
            "security", summary, "ReqPack could not determine exact package version for vulnerability matching.",
            "Request an explicit version, improve plugin version resolution, or relax unresolved-version policy.", {},
            finding.source, "version");
    }

    return make_error_diagnostic(
        "security", summary, "Matched vulnerability data blocks execution under current security policy.",
...
std::vector<Request> resolve_audit_requests(Executer* executor, const std::vector<Request>& requests) {
    ...
    for (Request& request : resolved) {
        if (request.action != ActionType::AUDIT || request.system.empty() || request.usesLocalTarget ||
...
void log_missing_sbom_packages(const std::vector<std::string>& missingPackages) {
    for (const std::string& packageSpecifier : missingPackages) {
        Logger::instance().diagnostic(make_error_diagnostic(
            "sbom", "sbom missing package: " + packageSpecifier,
            ...
            {}, "sbom", "export", {{"package", packageSpecifier}}));
    }
}
...
void log_validation_blocked(const std::vector<ValidationFinding>& findings) {
    Logger::instance().diagnostic(make_error_diagnostic(
        ...
        "Review reported findings, adjust requested versions, or relax security policy only if that risk is "
        "acceptable.",
        {}, "validator", "policy"));
    std::size_t shown = 0;
    for (const ValidationFinding& finding : findings) {
        if (finding.kind != "vulnerability" && finding.kind != "sync_error" &&
            finding.kind != "unsupported_ecosystem" && finding.kind != "unresolved_version") {
            continue;
        }
