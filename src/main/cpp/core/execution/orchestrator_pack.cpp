...
CommandOutput builtin_pack_output(const RqPackageBuildResult& result) {
    ...
    output.blocks.push_back(make_command_field_value_block({
        ...
        {.key = "Version",
         .value = result.metadata.version + "-" + std::to_string(result.metadata.release) + "+r" +
                  std::to_string(result.metadata.revision)},
    ...
    try {
        ...
        if (registry->getPlugin(request.system) == nullptr || !registry->loadPlugin(request.system)) {
            Logger::instance().diagnostic(make_error_diagnostic(
                "pack", "Plugin load failed for system '" + request.system + "'",
                "ReqPack could not load requested plugin before running pack operation.",
                "Check plugin installation and registry state, then retry.", {}, request.system, "pack"));
            return 1;
        }
        ...
        if (plugin == nullptr || !plugin->supportsPack()) {
            Logger::instance().diagnostic(
                make_error_diagnostic("pack", "system '" + request.system + "' does not support pack",
        ...
        if (ok && request.outputPath.empty() && artifacts.empty()) {
            Logger::instance().diagnostic(make_error_diagnostic(
                "pack", "plugin pack succeeded but did not register any output artifact",
        ...
        if (ok && !request.outputPath.empty() && artifacts.empty()) {
            Logger::instance().diagnostic(make_error_diagnostic(
                "pack", "plugin pack succeeded but output artifact is missing: " + request.outputPath,
                "Plugin reported success for explicit output path, but resulting artifact file was not found.",
                "Check plugin pack implementation and output path handling, then retry.", {}, request.system, "pack"));
            return 1;
        }
        if (!ok) {
            return 1;
        ...
        Logger::instance().diagnostic(
            make_error_diagnostic("pack", error.what(), "Pack command failed before producing valid artifact.",
                                  "Check package layout, plugin support, and output path, then retry.", {},
                                  request.system.empty() ? "rqp" : request.system, "pack"));
        return 1;
    }
}
