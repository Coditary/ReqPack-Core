...
std::vector<std::string> collect_request_item_ids(const std::vector<Request>& requests, const Registry& registry) {
	...
	for (const Request& request : requests) {
		...
		for (const std::string& specifier : request.packages) {
			...
			const std::string version = (versionSeparator == std::string::npos || versionSeparator + 1 >= specifier.size())
				? std::string{}
...
void display_already_satisfied_session(const std::vector<Request>& requests, const Registry& registry, ActionType action, int skippedCount) {
...
RunResultJsonDocument build_already_satisfied_json(
...
void ensure_request_plugins_discovered(const std::vector<Request>& requests, Registry* registry, const ReqPackConfig& config) {
	if (registry == nullptr) {
		return;
	}
	...
	if (!error && workspacePluginDirectory != configuredPluginDirectory && std::filesystem::exists(workspacePluginDirectory)) {
