...
std::vector<std::string> normalize_owners(std::vector<std::string> owners) {
    owners.erase(std::remove_if(owners.begin(), owners.end(), [](const std::string& owner) { return owner.empty(); }),
                 owners.end());
...
std::string install_method_from_owners(const std::vector<std::string>& owners) {
    const bool hasExplicit = std::any_of(owners.begin(), owners.end(),
                                         [](const std::string& owner) { return owner_has_prefix(owner, "root"); });
    const bool hasDependency = std::any_of(owners.begin(), owners.end(),
                                           [](const std::string& owner) { return owner_has_prefix(owner, "pkg"); });
