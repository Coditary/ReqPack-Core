...
InstalledStateEnvironment open_installed_state_environment(const std::filesystem::path& dbDirectory,
...
std::vector<InstalledEntry> load_entries_for_keys(MDB_txn* transaction, MDB_dbi dbi,
    ...
    for (const std::string& key : keys) {
        ...
        if (const std::optional<InstalledEntry> entry =
                history_manager_internal::deserialize_installed_entry(payload.value())) {
...
std::optional<std::pair<std::string, InstalledEntry>> find_installed_state_entry(MDB_txn* transaction, MDB_dbi dbi,
                                                                                 const std::string& system,
                                                                                 const std::string& name,
                                                                                 const std::string& version) {
    const std::vector<std::string> keys =
        collect_keys_with_prefix(transaction, dbi, history_manager_internal::installed_state_name_prefix(system, name));
    if (keys.empty()) {
        return std::nullopt;
    }
...
std::optional<InstalledEntry>
select_existing_entry_for_replace(const std::map<std::string, InstalledEntry>& existingByIdentity,
...
bool ensure_legacy_installed_state_imported(const InstalledStateEnvironment& environment,
    ...
    for (const InstalledEntry& entry : legacyEntries) {
        if (!put_value(transaction, environment.dbi,
                       history_manager_internal::installed_state_key(entry.system, entry.name, entry.version),
                       history_manager_internal::serialize_installed_entry(entry))) {
            mdb_txn_abort(transaction);
            return false;
        }
    }
...
std::vector<InstalledEntry> read_installed_entries(const InstalledStateEnvironment& environment) {
    ...
    while (result == MDB_SUCCESS) {
        ...
        if (history_manager_internal::starts_with(keyString, INSTALLED_STATE_KEY_PREFIX)) {
            ...
            if (const std::optional<InstalledEntry> entry =
                    history_manager_internal::deserialize_installed_entry(payload)) {
                entries.push_back(entry.value());
            }
        }
...
std::vector<InstalledEntry> load_installed_state_database(const std::filesystem::path& dbDirectory,
                                                          const std::filesystem::path& legacyPath) {
...
bool upsert_installed_state_database(const std::filesystem::path& dbDirectory, const std::filesystem::path& legacyPath,
    ...
    if (!put_value(transaction, environment.dbi, std::string(INSTALLED_STATE_INITIALIZED_KEY), "1") ||
        !put_value(transaction, environment.dbi,
                   history_manager_internal::installed_state_key(entry.system, entry.name, entry.version),
                   history_manager_internal::serialize_installed_entry(entry))) {
        mdb_txn_abort(transaction);
        return false;
...
bool remove_installed_state_database(const std::filesystem::path& dbDirectory, const std::filesystem::path& legacyPath,
                                     const std::string& system, const std::string& name, const std::string& version) {
    InstalledStateEnvironment environment = open_installed_state_environment(dbDirectory, true);
    if (!environment) {
        return false;
    }
...
bool replace_installed_state_database(const std::filesystem::path& dbDirectory, const std::filesystem::path& legacyPath,
                                      const std::string& system, const std::vector<InstalledEntry>& entries) {
    InstalledStateEnvironment environment = open_installed_state_environment(dbDirectory, true);
    if (!environment) {
        return false;
    }
    ...
    const std::vector<std::string> existingKeys = collect_keys_with_prefix(
        transaction, environment.dbi, history_manager_internal::installed_state_system_prefix(system));
    ...
    for (InstalledEntry entry : entries) {
        ...
        entry.system = system;
        const std::optional<InstalledEntry> existing =
            select_existing_entry_for_replace(existingByIdentity, existingByName, entry);
...
bool merge_installed_ownership_database(const std::filesystem::path& dbDirectory,
                                        const std::filesystem::path& legacyPath, const Package& package,
                                        const std::vector<std::string>& ownerIds, bool directRequest) {
    InstalledStateEnvironment environment = open_installed_state_environment(dbDirectory, true);
    if (!environment) {
        return false;
    }
    ...
    const std::optional<std::pair<std::string, InstalledEntry>> existing =
        find_installed_state_entry(transaction, environment.dbi, package.system, package.name, package.version);
    ...
    if (!put_value(transaction, environment.dbi, std::string(INSTALLED_STATE_INITIALIZED_KEY), "1") ||
        !put_value(transaction, environment.dbi, existing->first,
                   history_manager_internal::serialize_installed_entry(merged))) {
        mdb_txn_abort(transaction);
        return false;
    }
...
bool subtract_installed_ownership_database(const std::filesystem::path& dbDirectory,
                                           const std::filesystem::path& legacyPath, const Package& package,
    ...
    updated.owners.erase(std::remove_if(updated.owners.begin(), updated.owners.end(),
                                        [&](const std::string& owner) { return removedOwners.contains(owner); }),
                         updated.owners.end());
    ...
    if (!put_value(transaction, environment.dbi, std::string(INSTALLED_STATE_INITIALIZED_KEY), "1") ||
        !put_value(transaction, environment.dbi, existing->first,
                   history_manager_internal::serialize_installed_entry(updated))) {
        mdb_txn_abort(transaction);
        return false;
    }
...
bool HistoryManager::updateInstalledState(const HistoryEntry& entry) const {
    ...
    std::lock_guard<std::mutex> lock(mutex);
    if (isRemove) {
        return remove_installed_state_database(installedStateDatabasePath(), legacyInstalledStatePath(), entry.system,
                                               entry.packageName, entry.packageVersion);
    }

    if (isUpdate && !remove_installed_state_database(installedStateDatabasePath(), legacyInstalledStatePath(),
                                                     entry.system, entry.packageName, {})) {
        return false;
    }
...
bool HistoryManager::replaceInstalledState(const std::string& system,
                                           const std::vector<InstalledEntry>& entries) const {
    if (!config.history.trackInstalled) {
        return true;
    }
...
bool HistoryManager::mergeInstalledOwnership(const Package& package, const std::vector<std::string>& ownerIds,
    ...
    std::lock_guard<std::mutex> lock(mutex);
    return merge_installed_ownership_database(installedStateDatabasePath(), legacyInstalledStatePath(), package,
                                              ownerIds, directRequest);
}

bool HistoryManager::subtractInstalledOwnership(const Package& package,
                                                const std::vector<std::string>& ownerIds) const {
    if (!config.history.trackInstalled) {
        return true;
    }
    if (package.system.empty() || package.name.empty()) {
    ...
    std::lock_guard<std::mutex> lock(mutex);
    return subtract_installed_ownership_database(installedStateDatabasePath(), legacyInstalledStatePath(), package,
                                                 ownerIds);
}
