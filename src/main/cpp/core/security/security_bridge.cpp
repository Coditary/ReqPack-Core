...
void wire_reqpack_security_runtime() {
    ...
    set_security_stdout_callback([](const std::string& message) {
        Logger::instance().logStdout(message);
    });

    set_security_diagnostic_callback([](const std::string& category, const std::string& message, const std::string& hint) {
        Logger::instance().emitDiagnostic(make_error_diagnostic(category, message, message, hint));
    });

    set_security_flush_callback([]() {
