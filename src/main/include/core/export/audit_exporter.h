...
class AuditExporter {
    ...
    std::string renderTable(const Graph& graph, const std::vector<ValidationFinding>& findings, bool colorizeSeverity,
                            bool disableWrap, bool wideTable) const;
  ...
  public:
    explicit AuditExporter(PluginMetadataProvider* metadataProvider = nullptr,
                           const ReqPackConfig& config = default_reqpack_config());

    std::string renderGraph(const Graph& graph, const std::vector<ValidationFinding>& findings,
